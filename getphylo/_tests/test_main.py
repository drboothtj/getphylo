...
class TestCheckSeed(unittest.TestCase):
    ...
    def test_check_seed_checkpoints(self):
        ...
        with patch.object(glob, 'glob', return_value=glob1):
            assert check_seed(checkpoint1, search_string) == 'one.gbk'
        with patch.object(glob, 'glob', return_value=glob1):
            assert check_seed(checkpoint2, search_string) == 'one.gbk'
        with patch.object(glob, 'glob', return_value=glob1):
            with self.assertRaisesRegex(BadSeedError, 'checkpoint has been set'):
                check_seed(checkpoint3, search_string) == 'one.gbk'
        with patch.object(glob, 'glob', return_value=glob1):
            with self.assertRaisesRegex(BadSeedError, 'checkpoint has been set'):
                check_seed(checkpoint4, search_string) == 'one.gbk'
        with patch.object(glob, 'glob', return_value=glob1):
            with self.assertRaises(TypeError):
                check_seed(checkpoint5, search_string) == 'one.gbk'
        with patch.object(glob, 'glob', return_value=glob1):
            with self.assertRaises(TypeError):
                check_seed(checkpoint6, search_string) == 'one.gbk'
    ...
    def test_check_seed_glob(self):
        ...
        with patch.object(glob, 'glob', return_value=glob2):
            with self.assertRaisesRegex(BadSeedError, 'No files found'):
                check_seed(checkpoint1, search_string) 
